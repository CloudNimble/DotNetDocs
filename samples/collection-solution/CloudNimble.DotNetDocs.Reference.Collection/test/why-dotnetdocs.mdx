...
It happened with my own code. It happened with other libraries. Babysitting the AI so it doesn't take a wrong turn. Explaining concepts to the AI when it crashed out. Over and over 
...
A full decade since that tweet, how are .NET developers to follow that advice? GitHub Copilot can fill in your XML doc comments, but there's been
...
Six weekends later, the first release is finally ready.

Now *every* .NET developer can write better documentation.
