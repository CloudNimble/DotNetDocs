...
Extracts metadata from a single .NET assembly and its XML documentation file, building an in-memory model
            ([DocAssembly](DocAssembly)) with interconnected types, members, and parameters. Supports conceptual content
...
Initializes a new instance of [AssemblyManager](AssemblyManager) for a specific assembly.
...
A task representing the asynchronous operation, containing the [DocAssembly](DocAssembly) model.
