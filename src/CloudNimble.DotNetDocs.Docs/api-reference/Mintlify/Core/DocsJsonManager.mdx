...
The  instance loaded from the file system or string content.
...
A collection of  instances representing validation errors,
...
This method scans the specified directory recursively and builds a navigation structure
            based on the folder hierarchy and MDX files found. The method includes several advanced features:Directory Exclusions: Automatically excludes 'node_modules', 'conceptual', 
            'overrides', and any directories starting with '.' (dot directories).File Processing: Only includes .mdx files by default. .md files 
            generate warnings and are excluded from navigation.Index File Priority: Files named 'index' appear first in each directory's navigation.Enhanced Sorting: Multi-level sorting with index files first, 
            then regular files, then directories, all sorted alphabetically within their category.Navigation Override: If a directory contains 'navigation.json', 
            the user assumes complete control over that directory and all subdirectories. The JSON file should 
            contain a complete GroupConfig object.The method preserves the directory structure in the navigation while applying these intelligent 
