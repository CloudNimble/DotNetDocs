...
Extracts metadata from a single .NET assembly and its XML documentation file, building an in-memory model
            (`DocAssembly`) with interconnected types, members, and parameters. Supports conceptual content
...
Initializes a new instance of [AssemblyManager](../AssemblyManager.md) for a specific assembly.
