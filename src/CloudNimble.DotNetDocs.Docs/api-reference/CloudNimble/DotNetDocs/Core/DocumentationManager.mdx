...
This class manages the complete documentation lifecycle including enrichment,
            transformation, and rendering. It handles  creation,
...
```csharp
public System.Threading.Tasks.Task CreateConceptualFilesAsync(string assemblyPath, string xmlPath, string outputPath)
```
...
| `outputPath` | `string` | The base output path for conceptual files. |
...
```csharp
public System.Threading.Tasks.Task CreateConceptualFilesAsync(System.Collections.Generic.IEnumerable<(string, string)> assemblies, string outputPath)
```
...
Disposes of all cached  instances.
