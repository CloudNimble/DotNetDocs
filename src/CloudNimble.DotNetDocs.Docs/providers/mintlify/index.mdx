...
Customize MDX generation using Handlebars templates:

- Override default templates for types, members, or entire pages
- Access full object model in templates
- Inject custom HTML, CSS, or React components
- Reuse templates across multiple documentation projects

```csharp
services.AddDotNetDocsMintlify(options =>
{
    options.Template = "MyCustomTemplate.hbs";
...
<Steps>
  ...
  <Step title="Configure Your Pipeline">
    ...
    ```csharp With MSBuild Integration
    <PropertyGroup>
      <GenerateDotNetDocs>true</GenerateDotNetDocs>
      <DotNetDocsRenderer>Mintlify</DotNetDocsRenderer>
      <DotNetDocsOutputPath>$(MSBuildProjectDirectory)/docs</DotNetDocsOutputPath>
    </PropertyGroup>
    ```
    </CodeGroup>
  </Step>
...
<CardGroup cols={2}>
  ...
  <Card title="Leverage Custom Templates" icon="file-code">
    For unique documentation needs, create custom Handlebars templates rather than post-processing generated MDX files.
  </Card>
</CardGroup>
...
<CardGroup cols={2}>
  ...
  <Card title="Template Customization" icon="wand-magic-sparkles" href="/docs/customization/templates">
    Learn how to create custom Handlebars templates
  </Card>
  <Card title="Pipeline Configuration" icon="pipeline" href="/docs/concepts/pipeline">
