...
namespace CloudNimble.DotNetDocs.Core.Renderers.YamlConverters
{
    ...
    /// <summary>
    /// Custom type converter for Accessibility enum.
    /// </summary>
    public class AccessibilityTypeConverter : IYamlTypeConverter
    {
        ...
        public bool Accepts(Type type) => type == typeof(Accessibility);
        ...
        public object? ReadYaml(IParser parser, Type type, ObjectDeserializer rootDeserializer) 
            => throw new NotImplementedException("Reading Accessibility from YAML is not supported");
        ...
        {
            if (value is Accessibility accessibility)
            {
                emitter.Emit(new Scalar(accessibility.ToString()));
            }
            else
            {
